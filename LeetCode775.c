...
static int ContarMaiores( no_t *raiz, int valor ) {
	if ( !raiz ) {
		return 0;
	}

	if ( valor < raiz->valor ) {
		return 1 + Tamanho( raiz->dir ) +
...
bool isIdealPermutation( int *nums, int numsSize ) {
	...
	for ( i = 0; i < numsSize; i++ ) {
		...
		/* Conta quantos elementos anteriores sao maiores. */
		global += ContarMaiores( raiz, nums[i] );

		/* Insere o elemento atual na arvore. */
		raiz = Inserir_r( raiz, nums[i] );

		/* Conta inversoes locais. */
